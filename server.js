  ...
  let query = db.query(sql, async(err, result) => {
      if(err) throw err;
        ...
        if(err) throw err;
            ...
            let sql = `SELECT name,phone,address,SUM(Amount) AS total_amount FROM donors LEFT JOIN donations ON donors.donor_id = donations.donor GROUP BY donations.donor ORDER BY Amount DESC LIMIT 5 `;
